...
def test_release_version_and_provenance_are_consistent() -> None:
    ...
    server = (ROOT / "src" / "mlip_lsp" / "server.py").read_text(encoding="utf-8")

    assert _project_version() == RELEASE_VERSION
    assert mlip_lsp.__version__ == RELEASE_VERSION
    ...
    assert manifest["traceabilityReportPath"] == "reports/docstring-wiki-raw-traceability.json"
    assert f'version="{RELEASE_VERSION}"' in server
...
def test_release_docs_and_smoke_cover_acceptance_surface() -> None:
    ...
    for required in (
        ...
        "tests/fixtures/invalid/missing_model.json",
