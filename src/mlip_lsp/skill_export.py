...
SKILL_SPEC_JSON = (
    '{"schema":"scientific-lsp-skill/v1","name":"mlip","software":"mlip","display'
    '_name":"MLIP","description":"MLIP workflow preflight for manifests and ASE s'
    'cripts.","package":{"name":"mlip-lsp","install":"pip install mlip-lsp"},"ent'
    'rypoints":{"server":"mlip-lsp","tool":"mlip-lsp-tool"},"file_patterns":["*.m'
    'lip.json","*.mlip.yaml","*.mlip.yml","mlip.json","mlip.yaml","mlip.yml"],"op'
    'erations":["capabilities","check","context","complete","hover","symbols","fi'
    'x"],"diagnostic_contract":"DiagnosticEnvelope/v1","blocking_policy":{"mode":'
    '"fail-on-blocking","description":"Use --fail-on-blocking when generated inpu'
    'ts must be launch-ready."},"source_provenance":[{"kind":"official_docs","lab'
    'el":"MLIP documentation","url":"https://mlip.skoltech.ru/"}]}'
)
...
SKILL_MD_LINES = [
    '---',
    'name: mlip',
    'description: "MLIP workflow preflight for manifests and ASE scripts."',
    '---',
    '',
    '# MLIP LSP Skill',
    '',
    (
        'Use this skill when preparing, repairing, or reviewing MLIP input files befo'
    ...
    '',
    (
        '- Input patterns: *.mlip.json, *.mlip.yaml, *.mlip.yml, mlip.json, mlip.yaml'
        ', mlip.yml'
    ),
    '- Server command: `mlip-lsp`',
    '- Agent CLI: `mlip-lsp-tool`',
    '- Diagnostic contract: `DiagnosticEnvelope/v1`',
    '',
    '## Installing the checker',
    '',
    '```bash',
    'pip install mlip-lsp',
    '```',
    '',
    (
        'This installs the `mlip-lsp` language server and the `mlip-lsp-tool` agent C'
        'LI from the `mlip-lsp` Python package.'
    ),
    '',
    '## Useful inspection commands',
    '',
    '```bash',
    'mlip-lsp-tool capabilities',
    'mlip-lsp-tool skill-spec --format json',
    'mlip-lsp-tool skill-export --output ./skill',
    'mlip-lsp-tool check <input-file-or-dir> --format json',
    (
        'mlip-lsp-tool context <input-file-or-dir> --line 0 --character 0 --format js'
        'on'
    ),
    'mlip-lsp-tool hover <input-file-or-dir> --line 0 --character 0 --format json',
    (
        'mlip-lsp-tool complete <input-file-or-dir> --line 0 --character 0 --format j'
        'son'
    ),
    'mlip-lsp-tool symbols <input-file-or-dir> --format json',
    'mlip-lsp-tool fix <input-file-or-dir> --line 0 --character 0 --format json',
    '```',
    '',
    (
    ...
    '',
    '```bash',
    'mlip-lsp-tool check <input-file-or-dir> --format json --fail-on-blocking',
    '```',
    '',
    (
