...
def lsp_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="mlip-lsp")
    ...
    print(f"{'mlip-lsp'} scaffold: full JSON-RPC LSP is tracked in roadmap issues", file=sys.stderr)
    return 0


def lint_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="mlip-lint")
...
def fmt_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="mlip-fmt")
...
def test_main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="mlip-test")
