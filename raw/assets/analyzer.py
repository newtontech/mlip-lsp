"""MLIP-LSP Analyzer Module - Raw Source Extract

This file contains the core analysis logic from src/mlip_lsp/analyzer.py
"""
...
def analyze_path(path: Path) -> list[Diagnostic]:
    """Analyze a directory or file for MLIP diagnostics."""
    path = path.resolve()
...
def analyze_file(path: Path) -> list[Diagnostic]:
    """Analyze a single file for MLIP diagnostics."""
    try:
...
def _analyze_text(path: Path, content: str) -> list[Diagnostic]:
    """Analyze text files (NEP configs, etc.) for diagnostics."""
    diagnostics: list[Diagnostic] = []
...
def _meaningful_lines(content: str) -> list[tuple[int, str]]:
    """Extract meaningful (non-comment, non-empty) lines from content."""
